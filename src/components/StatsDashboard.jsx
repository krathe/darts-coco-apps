...
export default function StatsDashboard({ onBack }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { data } = await supabase.from('matches').select('*').eq('game_type', 'SOLO').order('created_at', { ascending: true });
    if (data) {
      setAllMatches(data);
      setAvailablePlayers([...new Set(data.map(m => m.winner_name))].sort());
      // Filtrer pour ne garder que les modes numériques (301, 501) pour l'instant
  ...
  const applyFilters = () => {
    ...
    if (gameMode !== 'ALL') {
       ...
       // Par défaut en 'ALL', on garde que les X01 classiques pour des stats cohérentes
  ...
  const processStats = (data) => {
    if (data.length === 0) {
      setStats({ games: 0, avg: 0, bestAvg: 0, total180: 0, bestLeg: 0, best301: 0, best501: 0, checkoutRate: 0, scoringDistribution: [], totalDartsThrown: 0, highestCheckout: 0, recentAvg: 0, avgTrend: 0 });
    ...
    // Calculs indépendants du filtre 'ALL' pour Best Legs
    ...
    setStats({ games, avg, bestAvg, total180, bestLeg, best301, best501, checkoutRate, scoringDistribution: scoringData, totalDartsThrown, highestCheckout, recentAvg, avgTrend });
        ...
        <div className="p-4 space-y-6 pb-12">
            ...
            <>
                    ...
                    <div className="flex items-center justify-between mb-4 relative z-10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Rang Actuel</div>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    {/* Grande Boite : Checkout avec Jauge Circulaire STATIQUE (Donut CSS) */}
